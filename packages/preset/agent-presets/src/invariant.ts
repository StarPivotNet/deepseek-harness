...
const install: InvariantInstaller = (ctx, fail) => {
  ...
  ctx.on('system-prompt/assemble', (_assembly, context, next) => {
    const presets = ctx.get('agentPresets') as { roots?: readonly unknown[]; composedPreset?: (scope: unknown) => unknown } | undefined
    const agent = context.agent
    if (presets !== undefined && (presets.roots?.length ?? 0) > 0
