...
function install(agent: Agent, ctx: Context, config: Required<Config>): () => void {
  ...
  try {
    ...
    register(scoped.tools.register(defineTool({
      name: 'wait_agent',
      description: 'Wait for the next teammate status, mailbox, or shared-task change after this call starts. Prefer ending the turn instead: when a teammate settles, its notice automatically starts the next Lead turn or steers a busy one. Use this tool only to sequence dependent steps on the next team change inside the current turn. This never wakes inactive members and returns noProgress immediately when no other member is running or provisioning. Re-list after wakeup or timeout instead of polling.',
      parameters: {
        timeout_ms: {
          type: 'integer',
