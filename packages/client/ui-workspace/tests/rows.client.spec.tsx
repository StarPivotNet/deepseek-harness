...
describe('workspace browser rows', () => {
  ...
  it.each([
    ...
    const result: SearchResultNode = {
      ...
      pendingInteraction, running: true, runningSubagentCount: 0, completed: false, hasActiveSchedule: false,
    }
  ...
  it('Chat bucket renders no workspace menu even when No Repo backs it', () => {
    const group: GroupNode = {
      key: '', workspaceId: undefined, cwd: undefined, createdAt: undefined, label: 'Chat',
      sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
    }
    render(<ProjectRowItem group={group} onToggle={vi.fn()} onCreate={vi.fn()} t={t} />)
  ...
  it('workspace and session context menus are independent of the ellipsis menus', () => {
    ...
    rerender(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRenameSession} onFork={onFork} onArchive={onArchive} t={t} />)
    ...
    expect(screen.getByRole('menuitem', { name: '归档任务' })).toBeTruthy()
    fireEvent.click(screen.getByRole('menuitem', { name: '分叉会话' }))
    expect(onFork).toHaveBeenCalledWith(node.id)
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0,
