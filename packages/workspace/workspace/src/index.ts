  ...
  private async createCanonical(canonical: string, title?: string, firstUse = false): Promise<WorkspaceEntity> {
    for (const entity of this.entities.values()) {
      if (entity.path === canonical || entity.folders.includes(canonical)) {
        ...
        const next = { ...current }
        let changed = false
        if (current.hiddenWorkspaceIds.includes(entity.id)) {
          next.hiddenWorkspaceIds = current.hiddenWorkspaceIds.filter(workspaceId => workspaceId !== entity.id)
          changed = true
        }
        if (firstUse && current.defaultWorkspaceId !== entity.id) {
          next.defaultWorkspaceId = entity.id
          changed = true
        }
        if (changed) await this.setState(next)
        return entity
      }
    }
