...
export class LlmRuntime extends TypertRemoteService {
  ...
  private normalizeModelInfo(
    ...
    if (systemPrompt !== undefined && typeof systemPrompt !== 'string') {
      throw new LlmError(
        `adapter returned invalid system prompt for provider "${provider}" model "${model}"`,
  ...
  async prepareCall(config: LlmCallConfig, signal?: AbortSignal): Promise<PreparedLlmCall> {
    ...
    return Object.freeze({
      ...
      ...modelInfo.systemPrompt === undefined ? {} : { systemPrompt: modelInfo.systemPrompt },
  ...
  private async * adapterStream(
    ...
    try {
      ...
      if (modelInfo.inputModalities !== undefined
        && !modelInfo.inputModalities.includes('video')
        && projectedMessages.some(message => contentHasVideo(message.content))) {
        projectedMessages = projectVideosForTextModel(projectedMessages)
      }
