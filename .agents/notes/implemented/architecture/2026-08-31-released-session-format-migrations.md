...
Team event payload versions are independent of Session format generations. The [V2 source audit](../../../../packages/session/session-format-v2-to-v3/README.md#source-audit) preserves both audited Team payload versions rather than relabeling historical events as current runtime events. Version-specific field validation retains malformed-record refusal; accepting a payload version alone does not establish compatibility.
