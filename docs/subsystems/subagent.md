...
The Agent inbox is the only queue. Parent-to-child messages use `Agent.steer()`. Child-to-parent messages read Host `subagent-delivery.reportBusy` at send time: a busy parent receives Steer at the nearest step or Queue in a later turn; an idle parent receives `followup()`. Successful delivery returns the accepted `MessageId`; durable `agent/inbox/spliced` events record inbox changes, and `user/message` records admission into model context.
