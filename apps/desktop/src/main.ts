...
async function main(): Promise<void> {
  ...
  const dockIconPng = readFileSync(
    development ? join(app.getAppPath(), 'resources', 'icon-windows.png') : join(process.resourcesPath, 'icon.png'),
  )
  let previousCompletedUnread = 0
  if (isMac) app.dock?.setIcon(nativeImage.createFromPath(
    development ? join(app.getAppPath(), 'resources', 'icon-windows.png') : join(process.resourcesPath, 'icon.png'),
  ))
